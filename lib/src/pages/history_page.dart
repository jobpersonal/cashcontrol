...
class _HistoryPageState extends State<HistoryPage> {
  ...
  void _setFilterDates(String type) {
    _startDate = (type == 'add')
        ...
        : _startDate.subtract(Duration(days: 7));
    _endDate = (type == 'add')
        ? _endDate.add(Duration(days: 7))
        : _endDate.subtract(Duration(days: 7));
  ...
  void _filterItems() {
    itemsTemp = items;
    final filtered = itemsTemp.where((element) =>
  ...
  Widget _buildColumnList(Size size) {
    return Container(
      ...
      child: ListView(
          children: List.generate(itemsTemp.length, (index) {
  ...
  Widget _buildListItem(
      {Size size, String title, String price, String hour, Color titleColor}) {
    return Container(
